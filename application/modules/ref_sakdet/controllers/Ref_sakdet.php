<?php
class Ref_sakdet extends CI_Controller{

    public function __construct() {
		parent::__construct();
		$this->atos_tiasa_leubeut();
		$this->load->model('m_sakdet');
	}
    ...
    public function index() {		
		...
		$data['kabupaten'] = $this->m_sakdet->get_kab_all();   
	...
	public function kab_index(){
		...
		$arr_kab = array();
		$query = $this->m_sakdet->super_masy_kab($id_kab);
		foreach ($query as $key) {
			$saksi = $this->m_sakdet->get_saksi($key->id);
			if($saksi == NULL){
			}else{
				$datap = [
					'ktp' => $key->ktp,
	...
	public function kec_index(){
		...
		$daerah = $this->input->post('daerah');
		$arr_kab = array();
		$query = $this->m_sakdet->super_masy_kec($id_kec);
		foreach ($query as $key) {
			$saksi = $this->m_sakdet->get_saksi($key->id);
			if($saksi == NULL){
			}else{
	...
	public function kel_index(){
		...
		$daerah = $this->input->post('daerah');
		$arr_kab = array();
		$query = $this->m_sakdet->super_masy_kel($id_kel);
		foreach ($query as $key) {
			$saksi = $this->m_sakdet->get_saksi($key->id);
			if($saksi == NULL){
			}else{
	...
	public function kab_excel(){
		...
		$data['saksi'] = $arr_kab;
		$data['filename'] = "Rekap Saksi per Kabupaten";
	...
	public function kec_excel(){
		...
		$data['saksi'] = $arr_kab;
		$data['filename'] = "Rekap Saksi per Kecamatan";
	...
	public function kel_excel(){
		...
		$arr_kab = array();
		$query = $this->m_sakdet->super_masy_kab($id_kel);
		foreach ($query as $key) {
			$saksi = $this->m_sakdet->get_saksi($key->id);
			if($saksi == NULL){
			}else{
		...
		$data['saksi'] = $arr_kab;
		$data['filename'] = "Rekap Saksi per Kelurahan";
