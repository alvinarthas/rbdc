<?php
class Vis_suara extends CI_Controller{

    public function __construct() {
		parent::__construct();
		$this->load->model('m_visuara');
    }
    ...
    public function index() {
		...
		$data['kabupaten'] = $this->m_visuara->get_kab_all();
	...
	public function get_jenis(){
		...
		$query = $this->m_visuara->get_tbl($kategori);
	...
	public function all_kat(){
		...
		$kabupaten = $this->m_visuara->get_kab_all();
		foreach ($kabupaten as $kab) {
			$kecamatan = $this->m_visuara->get_kec($kab->id);
			foreach ($kecamatan as $kec) {
				$kelurahan = $this->m_visuara->get_kel($kec->id);
				foreach ($kelurahan as $kel) {
					$target += $kel->target;
					$survey = $this->m_visuara->get_survey($kel->id);
		...
		$total =$this->m_visuara->count_allmasy();
	...
	public function all_job(){
		...
		foreach ($kabupaten as $kab) {
			...
			foreach ($kecamatan as $kec) {
				...
				foreach ($kelurahan as $kel) {
					$target += $kel->target;
					$masy = $this->m_visuara->get_masy($kategori,$jenis2,$kel->id);
					foreach ($masy as $mas) {
						$survey = $this->m_visuara->get_row_survey($mas->id);
		...
		$data['piedata'] = json_encode($pie_data);
		$data['piedata2'] = json_encode($pie_data2);
		$query = $this->m_visuara->row_tbl($kategori,$jenis2);
	...
	public function kat_all(){
		...
		$pie_kab2 = array();
		$pie_data = array();$pie_data2 = array();
		$kabupaten = $this->m_visuara->get_kab($id_kab);
		foreach ($kabupaten as $kab) {
			if($daerah == "kabupaten"){
				$kecamatan = $this->m_visuara->get_kec($kab->id);
			}else{
				$kecamatan = $this->m_visuara->get_kec_id($id_kec);
			}

			foreach ($kecamatan as $kec) {
				if($daerah == "kelurahan"){
					$kelurahan = $this->m_visuara->get_kel_id($id_kel);
				}else{
					$kelurahan = $this->m_visuara->get_kel($kec->id);
				}
			...
			array_push($pie_kab,$datap);
			if($daerah == "kabupaten"){
				$total =$this->m_visuara->count_daerahmasy($daerah,$id_kab);
			}elseif ($daerah =="kecamatan") {
				$total =$this->m_visuara->count_daerahmasy($daerah,$id_kec);
			}elseif ($daerah =="kelurahan") {
				$total =$this->m_visuara->count_daerahmasy($daerah,$id_kel);
			}
			$sisa = $total-$total_calon;
			$datap['country'] = "Total Yang Belum Melakukan Survey";
			$datap['value'] = $sisa;
	...
	public function kat_job(){
		...
		foreach ($kabupaten as $kab) {
			...
			array_push($pie_kab,$datap);
			if($daerah == "kabupaten"){
				$total =$this->m_visuara->count_katmasy($kategori,$jenis2,$daerah,$id_kab);
			}elseif ($daerah =="kecamatan") {
				$total =$this->m_visuara->count_katmasy($kategori,$jenis2,$daerah,$id_kec);
			}elseif ($daerah =="kelurahan") {
				$total =$this->m_visuara->count_katmasy($kategori,$jenis2,$daerah,$id_kel);
			}
	...
	public function kenal_kat_all(){
		...
		foreach ($kabupaten as $kab) {
			...
			foreach ($kecamatan as $kec) {
				...
				foreach ($kelurahan as $kel) {
					$surveyya = $this->m_visuara->get_ya_survey_all($kel->id);
					$kenal_ya += $surveyya->ya;
					$surveytidak = $this->m_visuara->get_tidak_survey_all($kel->id);
		...
		foreach ($kabupaten as $kab) {
			...
			foreach ($kecamatan as $kec) {
				...
				foreach ($kelurahan as $kel) {
					$masy = $this->m_visuara->get_masy($kategori,$jenis2,$kel->id);
					foreach ($masy as $mas) {
						$surveyya = $this->m_visuara->get_ya_survey_all($mas->id);
						$kenal_ya += $surveyya->ya;
						$surveytidak = $this->m_visuara->get_tidak_survey_all($mas->id);
						$kenal_tidak += $surveytidak->tidak;
					}
				}
			}
