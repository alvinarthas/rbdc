...
class Ref_calon extends MX_Controller{
    // Add Construct
	...
	public function index( $offset = 0 ) {
        ...
        $per_page = 20;  
        $qry = "SELECT * FROM tbl_calon";

        if ($this->session->userdata('s_cari_global')!="") {
            $qry.="  WHERE nm_cagub like '%".$this->db->escape_like_str($this->session->userdata('s_cari_global'))."%'  ";
        } 
        ...
        $config['base_url']= base_url().'/ref_calon/index'; 
        ...
        $data['sub_judul_form']="Data Calon Gubernur dan Wakil Gubernur ";
	...
	public function tambih() {
		...
		$data['sub_judul_form']="Cagub dan Cawagub";
	   ...
	   try {				
            ...
            if($id==''|| $id==null){
                ...
                if(isset($_FILES['file'])){
                    $config['file_name']        = 'calon_'.$id;
                    $config['upload_path']      = './assets_users/paslon';
                    ...
                    $config['max_size']         = 4048;
                        ...
                        $this->load->library('upload', $config);
                            ...
                            $data = array(
                                'nm_cagub' => $this->input->post('nm_cagub'),
                                'nm_cawagub' => $this->input->post('nm_cawagub'),	
                                'no_urut' => $this->input->post('no_urut'),
                                'gambar' => $namafile['file_name'],
                            );
                            $xss_data = $this->security->xss_clean($data);
                            $this->db->insert('tbl_calon', $xss_data);
                ...
                if(isset($_FILES['file'])){
                    $q=$this->db->query("SELECT * FROM tbl_calon WHERE id =".$id);
                    $data=$q->row();
                    $file ='./assets_users/paslon/'.$data->gambar;
                        ...
                        try {
                            ...
                            $this->load->library('upload', $config);
                                if ( ! $this->upload->do_upload('file')){
                                    ...
                                    $xss_data = $this->security->xss_clean($data);
                                    $this->db->where('id',$id);
                                    $this->db->update('tbl_calon',$xss_data);
   ...
   public function robih() {
	   ...
	   $sql = "SELECT * FROM tbl_calon  WHERE id='".$id."' ";
